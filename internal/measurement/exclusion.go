...
func Reconcile(batchID string, ms []*model.SkewMeasurement, exemptions []*model.Exemption,
	condsByExemption map[string][]*model.ExemptionCondition, statusByEx map[string]string,
	...
	for _, m := range ms {
		...
		for _, ex := range exemptions {
			...
			if st == model.ExemptionStatusRevoked || st == model.ExemptionStatusInvalid {
				continue
			}
		...
		case hasExemptionPair(m, exemptions, statusByEx):
...
func hasExemptionPair(m *model.SkewMeasurement, exemptions []*model.Exemption, statusByEx map[string]string) bool {
	for _, ex := range exemptions {
		st := statusByEx[ex.ID]
		if st == "" {
			st = ex.Status
