...
func TestRevokeAfterFailuresReturnsPending(t *testing.T) {
	h, p := newProbeHandler(t)
	pb := p.setupBatch(t)
	pb.importMeasurement(t, p, pb.fnMode.ID, 50, "2026-08-27T08:00:00Z")
	pb.importMeasurement(t, p, pb.scMode.ID, 950, "2026-08-27T08:05:00Z")
	ex := pb.createExemption(t, p, "EX-1", 100, pb.fnMode, pb.scMode)
	pb.open(t, p)

	if w := postJSON(t, h, "/api/batches/"+pb.batch.ID+"/validate", nil); w.Code != http.StatusOK {
		t.Fatalf("validate: %d %s", w.Code, w.Body.String())
	}
	b, err := p.bs.Get(pb.batch.ID)
	if err != nil {
		t.Fatal(err)
	}
	if b.Status != model.BatchStatusFailures {
		t.Fatalf("batch status=%s want has_failures", b.Status)
	}

	if w := postJSON(t, h, "/api/exemptions/"+ex.ID+"/revoke", nil); w.Code != http.StatusOK {
	...
	if b.Status != model.BatchStatusPending {
		t.Fatalf("after revoke batch status=%s want pending", b.Status)
	}

	if w := postJSON(t, h, "/api/batches/"+pb.batch.ID+"/validate", nil); w.Code != http.StatusOK {
		t.Fatalf("revalidate: %d %s", w.Code, w.Body.String())
	}
	b, err = p.bs.Get(pb.batch.ID)
	if err != nil {
		t.Fatal(err)
	...
	if b.Status != model.BatchStatusPublishable {
		t.Fatalf("after revalidate batch status=%s want publishable", b.Status)
	}
}
