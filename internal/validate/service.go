...
func (s *Service) Validate(batchID string) (*Result, error) {
	run, resumed, err := s.vs.BeginRun(batchID, func() *model.ValidationRun {
	...
	// 测量状态对账（matched / mode_mismatch）。
	if err := measurement.Reconcile(batchID, measurements, exemptions, condsByEx,
