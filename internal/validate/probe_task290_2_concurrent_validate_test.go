...
func TestConcurrentValidateSingleRun(t *testing.T) {
	p := newProbeEnv(t)
	pb := p.setupBatch(t)
	...
	go func() {
		for {
			...
			case <-stopPoll:
				return
			default:
				runs, err := p.vs.Runs(pb.batch.ID)
				if err != nil {
					return
				}
				n := 0
				...
				for {
					...
					if int32(n) <= old || atomic.CompareAndSwapInt32(&peak, old, int32(n)) {
						break
					}
				}
	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			defer wg.Done()
			<-start
			if _, err := p.va.Validate(pb.batch.ID); err != nil {
				errCh <- err
			}
		}()
	}
	...
	if got := atomic.LoadInt32(&peak); got > 1 {
		t.Fatalf("peak concurrent running runs=%d want<=1", got)
	}
}
