...
func TestInvalidExemptionDoesNotMarkMatched(t *testing.T) {
	p := newProbeEnv(t)
	pb := p.setupBatch(t)
	pb.importMeasurement(t, p, pb.scMode.ID, 950, "2026-08-27T08:05:00Z")
	pb.createExemption(t, p, "EX-1", 100, pb.fnMode, pb.scMode)
	pb.open(t, p)
	...
	for _, m := range items {
		if m.ModeID == pb.scMode.ID && m.Status == model.MeasurementStatusMatched {
			t.Fatalf("skew_exceeded measurement %s must not be matched, got %s", m.ID, m.Status)
		}
	}
}
