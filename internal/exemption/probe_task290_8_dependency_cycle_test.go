...
func TestAddDependencyRejectsCycle(t *testing.T) {
	...
	if err := p.ex.AddDependency(exB.ID, exA.ID); err != nil {
		t.Fatal(err)
	}
	if err := p.ex.AddDependency(exA.ID, exB.ID); err != model.ErrRuleCycle {
		t.Fatalf("AddDependency A->B err=%v want ErrRuleCycle", err)
	}
}
