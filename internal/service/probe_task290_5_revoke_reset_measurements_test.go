...
func TestRevokeResetsMeasurementStatuses(t *testing.T) {
	p := newProbeEnv(t)
	pb := p.setupBatch(t)
	...
	// 模拟上一轮验证留下的派生测量状态（撤销复验前应被 ResetStatuses 清回 raw）。
	items, err := p.ms.List(pb.batch.ID)
	if err != nil {
		t.Fatal(err)
	}
	for _, m := range items {
		if err := p.ms.UpdateStatus(m.ID, model.MeasurementStatusMatched, measurement.ReasonMatched); err != nil {
			t.Fatal(err)
		}
	}
	if _, err := p.orch.RevokeExemption(ex.ID); err != nil {
		t.Fatal(err)
	}
	...
	for _, m := range items {
		if m.Status != model.MeasurementStatusRaw {
			t.Fatalf("measurement %s status=%s want raw after revoke", m.ID, m.Status)
		}
	}
}
