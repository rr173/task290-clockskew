...
func TestValidateErrorKeepsBatchPending(t *testing.T) {
	p := newProbeEnv(t)
	pb := p.setupBatch(t)
	exA := pb.createExemption(t, p, "EX-A", 100, pb.fnMode)
	exB := pb.createExemption(t, p, "EX-B", 100, pb.fnMode)
	pb.open(t, p)

	if err := p.es.AddDependency(&model.ExemptionDependency{
		ID: model.NewID("dep"), ExemptionID: exA.ID, DependsOn: exB.ID,
	}); err != nil {
		t.Fatal(err)
	}
	if err := p.es.AddDependency(&model.ExemptionDependency{
		ID: model.NewID("dep"), ExemptionID: exB.ID, DependsOn: exA.ID,
	}); err != nil {
		t.Fatal(err)
	}

	_, err := p.orch.ValidateBatch(pb.batch.ID)
	if err == nil {
		t.Fatal("expected validate error for dependency cycle")
	}
	...
	if b.Status != model.BatchStatusPending {
		t.Fatalf("batch status=%s want pending after validate error", b.Status)
	}
}
