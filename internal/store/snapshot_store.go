...
func (s *SnapshotStore) Published(batchID string) (*model.ValidationSnapshot, error) {
	rows, err := s.db.Query(
		`SELECT id, batch_id, name, status, content_hash, frozen_corners, created_at, published_at
		 FROM validation_snapshots WHERE batch_id = ? AND status IN (?, ?) ORDER BY created_at ASC LIMIT 1`,
		batchID, model.SnapshotStatusPublished, model.SnapshotStatusSuperseded)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
